                                ...
                                {orders.length > 0 ? (
                                    orders.map((order, idx) => (
                                        ...
                                        >
                                            <td className="py-6 pl-4">
                                                <div className="flex items-center gap-4">
                                                    <div className="w-10 h-10 rounded-xl bg-brand-50 flex items-center justify-center text-brand-600 font-black text-xs">
                                                        {order.product?.name?.[0] || 'P'}
                                                    </div>
                                                    <div>
                                                        <span className="font-bold text-slate-900 block">{order.product?.name || `Product #${order.productId}`}</span>
                                                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">ID: {order.orderId}</span>
                                                    </div>
                                                </div>
                                            </td>
                                            ...
                                            <td className="py-6 text-sm font-black text-slate-900">{order.quantity}</td>
                                            <td className="py-6 text-sm font-black text-brand-600 tracking-tight">
                                                ₹ {order.totalPrice.toLocaleString()}
                                            </td>
                                            <td className="py-6">
                                                <span className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border shadow-sm ${order.deliveryStatus === 'DELIVERED' ? 'border-emerald-200 text-emerald-600 bg-emerald-50' :
                                                    order.deliveryStatus === 'CANCELLED' ? 'border-rose-200 text-rose-600 bg-rose-50' :
                                    ...
                                    <tr>
                                        <td colSpan={5} className="py-20 text-center">
                                            ...
                                            <p className="text-slate-500 text-sm mt-2 font-medium">Your marketplace activity will appear here.</p>
                                        </td>
                                    </tr>
                                )}
                            </tbody>
